...
export const initialState = {
  ...
  totalSaleTokens: 0,
  nonSaleEntities: [{entityName: "Unallocated", entityPercentage: 100, entityAddress: 'NA' }],
  entityName: "",
  entityPercentage: "",
  entityAddress: "",
};
  ...
  switch (action.type) {
    ...
    case actionTypes.ADD_NON_SALE_ENTITY: {
      ...
      if (action.payload) {
        ...
        if (total< 100){
          nonSaleEntities.push({entityName: "Unallocated", entityPercentage: 100-total, entityAddress: 'NA' })
        }
        return {
          ...state,
          nonSaleEntities: nonSaleEntities,
    ...
    case actionTypes.CALCULATE_TOKENS: {
      var round3Rate = 100
      var round2Rate = parseFloat(state.tokenPriceFactor).toFixed(1) * round3Rate
      var round1Rate = Math.pow(parseFloat(state.tokenPriceFactor).toFixed(1), 2) * round3Rate
    ...
    case actionTypes.ROUND1_TARGET_USD_CHANGED: {
      if (action.payload) {
        return {
          ...
          round1TargetEth: (parseFloat(action.payload) / parseFloat(state.ethPrice)).toFixed(4)
        };
      } else {
        return {
          ...state,
    ...
    case actionTypes.ROUND1_TARGET_ETH_CHANGED: {
      if (action.payload) {
        return {
          ...state,
          round1TargetUSD: parseFloat(action.payload) * parseFloat(state.ethPrice).toFixed(2),
    ...
    case actionTypes.ROUND2_TARGET_USD_CHANGED: {
      if (action.payload) {
        return {
          ...
          round2TargetEth: (parseFloat(action.payload) / parseFloat(state.ethPrice)).toFixed(4)
        };
      } else {
        return {
          ...state,
    ...
    case actionTypes.ROUND2_TARGET_ETH_CHANGED: {
      if (action.payload) {
        return {
          ...state,
          round2TargetUSD: parseFloat(action.payload) * parseFloat(state.ethPrice).toFixed(2),
    ...
    case actionTypes.ROUND3_TARGET_USD_CHANGED: {
      if (action.payload) {
        return {
          ...
          round3TargetEth: (parseFloat(action.payload) / parseFloat(state.ethPrice)).toFixed(4)
        };
      } else {
        return {
          ...state,
    ...
    case actionTypes.ROUND3_TARGET_ETH_CHANGED: {
      if (action.payload) {
        return {
          ...state,
          round3TargetUSD: parseFloat(action.payload) * parseFloat(state.ethPrice).toFixed(2),
